...
import { createPageDetector, createRouteAction, screen, width, height, getTemplate, imageNameParser, ocrText, toScreenX, toScreenY, ocrRegion } from '../utils/img'
...
export class 再别前线废土互市 extends BasePage {
  ...
  收购价(): boolean {
    ...
    // 调用提取函数
    var text = ocrRegion(tmp, 0, 0, tmp.getWidth(), tmp.getHeight())?.text || '';
    //var text = ocrText(tmp, 0, 0, tmp.getWidth(), tmp.getHeight())
    ...
    sleep(300)
    click(toScreenX(width - 50), toScreenY(height - 50))
