...
## 2026-08-15: 弹窗归属页判定（选择技能 → 战斗中）

### 背景

战斗中技能弹窗弹出时，暂停按钮识别偶发失败（动画遮罩/动态元素干扰，阈值 0.9 匹配不过），
"战斗中" is() 返回 false → detectCurrentPage 只剩"选择技能"匹配。
选择技能无 routes() 出口 → findPath 失败 → 回退关不掉弹窗 → 5 次回退全无效 → "回退死循环: 选择技能 无出口"。

关键事实：技能弹窗只在战斗中弹出（游戏机制）→ "识别到选择技能必在战斗中"恒成立，
与暂停按钮识别是否成功无关。

### 实现

1. BasePage 新增可选属性 `hostPage?: typeof BasePage`（弹窗归属页面声明，未声明页面行为不变）
2. 选择技能 声明 `hostPage = 战斗中`
3. go() 到达判定处（findPath 之前，零找图）：`current.hostPage === targetClass` → 视为已到达，
   弹窗留给调用方（battleHandler 等）处理

不采用方案：
- **选择技能加"→战斗中"路由**：executePath 轮询时选择技能先注册永远优先匹配，
  等不到战斗中出现，会超时后仍回退死循环
- **调整注册顺序（战斗中先注册）**：daily.ts 注释明确要求选择技能优先识别弹窗；
  Game.ts 已是战斗中先注册，暂停识别偶发失败时照样触发
- **is() 双信号补全**（战斗中.ts 注释与实现不符处）：弹窗遮罩下技能图标不一定识别得到，
  且不解决"选择技能被识别"时的死循环，只能当辅助

### 效果

| 场景 | 优化前 | 优化后 |
|------|--------|--------|
| go(战斗中) 遇技能弹窗（暂停识别偶发失败） | 5 轮回退 + 死循环异常 ~10s + 误点 | 0 次找图，直接成功 |

### 优化日志

| 版本 | 方案 | 问题 |
|------|------|------|
| — | **弹窗归属页判定**：`hostPage` 属性 + go() 到达判定 | 选择技能弹窗下 go(战斗中) 不再回退死循环 |
...
| 2026-08-17 | **弹窗关闭轮询 tryCloseModals 返回值门控**：未知页面轮询改为"点到弹窗→轮询 5 次；没点到→轮询 2 次兜底" | 无弹窗时 3.3s→约 1.9s（省 3 次全量 detect）；初版曾用 pageChange 像素门控，实测动态页 >3% 采样点差异恒成立、门控失效零收益，且存在"动画停止后漏识别"边界风险，已弃用；回归确定性信号（tryCloseModals 是否真点了按钮），无漏识别风险 |
...
## 2026-07-25: 未知页面弹窗提前关闭 + 盲等改轮询

### 背景

导航日志发现遇到未知页面时有多段盲等串联：

```
executePath 超时 → sleep(500) → 返外层 → 识别不到 → sleep(1000) "过渡等待"
→ 仍识别不到 → 关弹窗 → sleep(1500) → 识别成功 ← 弹窗早关了，还在等
```

三段盲等共 **500+1000+1500 = 3s**，实际弹窗关闭后页面很快可识别。
...
| 位置 | 之前 | 之后 |
|------|------|------|
| `go()` 过渡等待处理（~行 158） | `sleep(1000)` 盲目等待 | `tryCloseModals()` 先关弹窗 + `sleep(500)` |
| `go()` 未知页面处理（~行 73） | `tryCloseModals` + `sleep(1500)` 定长等待 | `tryCloseModals` + 轮询 `5×300ms`，提前识别提前退出 |
| `executePath()` 超时处理（~行 351） | `sleep(500)` | `sleep(300)` |

### 效果预期

| 场景 | 优化前 | 优化后 |
|------|--------|--------|
| 弹窗遮挡目标页（当前日志） | ~4.6s（500+1000+1500+检测） | ~1.1s（关弹窗 + 最多 5×300ms），约 **3.5s** 节省 |
| 纯加载过渡（无弹窗） | ~1.5s（1000+500） | ~0.8s（500+300），约 **0.7s** 节省 |