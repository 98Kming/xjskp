...
console.log('')
console.log('--- 战斗结果: 只进战斗段, 不动任务三态与明细 ---')
发布({ 类型: '本轮开始' })
战斗胜(); 战斗胜(); 战斗负()
var 战 = 本轮汇总()
assert(战.战斗成功 === 2 && 战.战斗失败 === 1,
  '胜/负计数 2/1, 实际 ' + 战.战斗成功 + '/' + 战.战斗失败)
assert(战.成功 === 0 && 战.跳过 === 0 && 战.异常 === 0,
  '战斗结果不计入任务三态, 实际 ' + 战.成功 + '/' + 战.跳过 + '/' + 战.异常)
assert(战.条数 === 0, '战斗不是"任务", 条数仍为 0, 实际 ' + 战.条数)
assert(战.明细.length === 0, '战斗结果不产生明细, 实际 ' + 战.明细.length)

console.log('')
console.log('--- 战斗结果: 计数条文案 ---')
assert(格式化计数条(战) === '✅ 2  ❌ 1', '战斗模式文案无 ⏭ 段, 实际 [' + 格式化计数条(战) + ']')

console.log('')
console.log('--- 战斗结果: 与日常计数互斥, 出了战斗结果就只显示战斗段 ---')
发布({ 类型: '本轮开始' })
成功('A'); 跳过('B'); 异常('C')
assert(格式化计数条(本轮汇总()) === '✅ 1  ⏭ 1  ❌ 1',
  '无战斗结果时仍走日常分支, 实际 [' + 格式化计数条(本轮汇总()) + ']')
战斗胜()
assert(格式化计数条(本轮汇总()) === '✅ 1  ❌ 0',
  '有战斗结果后只显示战斗段, 实际 [' + 格式化计数条(本轮汇总()) + ']')

console.log('')
console.log('--- 战斗结果: 全败也走战斗分支(不能用"成功数>0"当开关) ---')
发布({ 类型: '本轮开始' })
战斗负(); 战斗负()
assert(格式化计数条(本轮汇总()) === '✅ 0  ❌ 2',
  '全失败仍显示战斗段, 实际 [' + 格式化计数条(本轮汇总()) + ']')

console.log('')
console.log('--- 战斗结果: 本轮开始清零 ---')
发布({ 类型: '本轮开始' })
var 清零后 = 本轮汇总()
assert(清零后.战斗成功 === 0 && 清零后.战斗失败 === 0,
  '本轮开始后战斗计数归零, 实际 ' + 清零后.战斗成功 + '/' + 清零后.战斗失败)
assert(格式化计数条(清零后) === '✅ 0  ⏭ 0  ❌ 0',
  '清零后回到日常分支, 实际 [' + 格式化计数条(清零后) + ']')

console.log('')
console.log('--- 战斗结果: 初始态按模式区分 ---')
发布({ 类型: '本轮开始', 模式: '战斗' })
assert(本轮汇总().模式 === '战斗', '模式随本轮开始记下, 实际 ' + 本轮汇总().模式)
assert(格式化计数条(本轮汇总()) === '✅ 0  ❌ 0',
  '战斗模式开局即胜/负格式, 无 ⏭ 段, 实际 [' + 格式化计数条(本轮汇总()) + ']')
战斗胜(); 战斗负()
assert(格式化计数条(本轮汇总()) === '✅ 1  ❌ 1',
  '战斗模式计数, 实际 [' + 格式化计数条(本轮汇总()) + ']')

console.log('')
console.log('--- 战斗结果: 不传模式默认日常, 初始态不变 ---')
发布({ 类型: '本轮开始' })
assert(本轮汇总().模式 === '日常', '缺省模式=日常, 实际 ' + 本轮汇总().模式)
assert(格式化计数条(本轮汇总()) === '✅ 0  ⏭ 0  ❌ 0',
  '日常模式开局仍带 ⏭ 段, 实际 [' + 格式化计数条(本轮汇总()) + ']')

console.log('')
console.log('--- 战斗结果: 手工重置不带模式时回落日常 ---')
重置本轮()
assert(本轮汇总().模式 === '日常', '重置回落日常, 实际 ' + 本轮汇总().模式)

console.log('')
console.log('--- 战斗结果: 日常轮里混进战斗结果也切战斗段(兜底) ---')
发布({ 类型: '本轮开始' })
成功('A')
战斗胜()
assert(格式化计数条(本轮汇总()) === '✅ 1  ❌ 0',
  '兜底:有战斗结果即切战斗段, 实际 [' + 格式化计数条(本轮汇总()) + ']')
...
// ---------- 渲染冒烟: 计数条(战斗模式) ----------
// 放在通知冒烟之后:战斗结果一旦累计,计数条就切到战斗段,会打乱上面按日常文案写的断言
console.log('')
console.log('--- 渲染冒烟: 计数条(战斗模式) ---')
发布({ 类型: '本轮开始', 模式: '战斗' })
sleep(600)
var 读回的开局文字 = (global as any).__计数条文字
assert(读回的开局文字 === '✅ 0  ❌ 0',
  '战斗模式开局渲染即胜/负格式, 实际 [' + 读回的开局文字 + ']')

战斗胜(); sleep(700)
战斗负(); sleep(700)
战斗胜(); sleep(700)
var 读回的战斗文字 = (global as any).__计数条文字
assert(读回的战斗文字 === '✅ 2  ❌ 1',
  '战斗模式计数条文字与汇总一致, 实际 [' + 读回的战斗文字 + ']')
